            ...
            If v_strActype.Length > 0 Then
                ...
                If v_ds.Tables(0).Rows.Count = 1 Then
                    If v_ds.Tables(0).Rows(0)(0) > 0 Then
                        Return gc_ERRCODE_FO_ACTYPE_DUPLICATED
                    End If
                End If
            End If
    ...
    Overrides Function CheckBeforeDelete(ByVal v_strMessage As String) As Long
        ...
        Try
            ...
            ElseIf v_strLocal = "N" Then
                ...
                v_obj.NewDBInstance(gc_MODULE_HOST)
            End If
            'Kiểm tra xem Mã dữ liệu bị xoá có nằm trong bảng CIMAST khác hay không
            v_strSQL = "SELECT COUNT(ACTYPE) FROM FOMAST WHERE 0=0 AND " & v_strClause
            v_ds = v_obj.ExecuteSQLReturnDataset(CommandType.Text, v_strSQL)
            If v_ds.Tables(0).Rows.Count = 1 Then
                If v_ds.Tables(0).Rows(0)(0) > 0 Then
                    Return ERR_CI_CITYPE_ACTYPE_HAS_CONSTRAINT
                End If
            End If

            'Kiem tra ACTYPE co con du lieu lien quan trong bang ICCFTYPEDEF hay khong?
            v_strSQL = "SELECT COUNT(ACTYPE) FROM ICCFTYPEDEF WHERE MODCODE = 'FO' AND " & v_strClause
            v_ds = v_obj.ExecuteSQLReturnDataset(CommandType.Text, v_strSQL)
            If v_ds.Tables(0).Rows.Count = 1 Then
                If v_ds.Tables(0).Rows(0)(0) > 0 Then
                    Return gc_ERRCODE_FO_ACTYPE_CONTRAINTS
                End If
            End If
