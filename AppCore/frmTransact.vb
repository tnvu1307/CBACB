...
#Region " Overridable Function "
    ...
    Public Overridable Sub ShowInquiry(ByRef v_strTLTXCD As String, ByRef v_strXmlData As String)
        Dim v_strTempXMLData As String
        Dim v_xmlTempDocument As New Xml.XmlDocument, v_nodeTempList As Xml.XmlNodeList, v_nodeTemptxData As Xml.XmlNode
        Dim v_dataTempElement As Xml.XmlElement, v_entryTempNode As Xml.XmlNode
        ...
        'LogError.Write("Error source: 1171 ShowInquiry: " & v_strXmlData & "Error message: " & v_strXmlData, EventLogEntryType.Information)
        Try
            If Trim(mskTransCode.Text) = gc_OD_CLEARINGINQUIRY Then
                Dim frm As New frmClearingSchedule(Me.UserLanguage)
                ...
                frm.XmlObjData = v_strXmlData
                frm.ShowDialog()
                If Len(frm.RefValue) > 0 Then
                    'Kiá»ƒm tra náº¿u cÃ³ yÃªu cáº§u thanh toÃ¡n thÃ¬ náº¡p giao dá»‹ch thanh toÃ¡n tÆ°Æ¡ng á»©ng: RefValue=STSCHD.AUTOID
                    'Dá»±ng dá»¯ liá»‡u v_strXmlData & v_strTLTXCD Ä‘á»ƒ táº¡o giao dá»‹ch thanh toÃ¡n tÆ°Æ¡ng á»©ng
                    Dim v_xmlDocument As New Xml.XmlDocument, v_nodeList As Xml.XmlNodeList, v_nodetxData As Xml.XmlNode
                    Dim v_strValue, v_strAUTOID, v_strTempTLTXCD, v_strFLDNAME As String
                    v_xmlDocument.LoadXml(v_strXmlData)
                    v_nodeList = v_xmlDocument.SelectNodes("/TransactMessage/ObjData")
                    For i = 0 To v_nodeList.Count - 1
                        'Láº¥y giÃ¡ trá»‹ AUTOID
                        ...
                        For j = 0 To v_nodeList.Item(i).ChildNodes.Count - 1
                            With v_nodeList.Item(i).ChildNodes(j)
                                ...
                                ElseIf v_strFLDNAME = "DUETYPE" Then
                                    Select Case v_strValue
                                        Case "RM"
                                            v_strTempTLTXCD = gc_OD_CIRECEIVE
                                        Case "SM"
                                            v_strTempTLTXCD = gc_OD_CISEND
                                        Case "RS"
                                            v_strTempTLTXCD = gc_OD_SERECEIVE
                                        Case "SS"
                                            v_strTempTLTXCD = gc_OD_SESEND
                                    End Select
                                End If
                            End With
                        Next

                        'Loáº¡i bá»? Node náº¿u
                        If frm.RefValue = v_strAUTOID Then
                            'XÃ¡c Ä‘á»‹nh Ä‘Ãºng Node chá»©a dá»¯ liá»‡u
                            v_strTLTXCD = v_strTempTLTXCD
                            'Táº¡o dá»¯ liá»‡u v_strXmlData Ä‘á»ƒ náº¡p cho giao dá»‹ch káº¿ tiáº¿p
                            v_strTempXMLData = BuildXMLTxMsg(gc_MsgTypeTrans, gc_IsLocalMsg, v_strTLTXCD, Me.BranchId, Me.TellerId, Me.IpAddress, Me.WsName)
                            v_xmlTempDocument.LoadXml(v_strTempXMLData)
                            v_dataTempElement = v_xmlTempDocument.CreateElement(Xml.XmlNodeType.Element, "ObjData", "")
                            'Copy láº¡i Node cÅ©
                            For j = 0 To v_nodeList.Item(i).ChildNodes.Count - 1
                                With v_nodeList.Item(i).ChildNodes(j)
                                    'Append entry to data node
                                    v_nodeTemptxData = v_xmlTempDocument.CreateNode(Xml.XmlNodeType.Element, "entry", "")

                                    'Add field name
                                    v_attrFLDNAME = v_xmlTempDocument.CreateAttribute("fldname")
                                    v_attrFLDNAME.Value = CStr(CType(.Attributes.GetNamedItem("fldname"), Xml.XmlAttribute).Value)
                                    v_nodeTemptxData.Attributes.Append(v_attrFLDNAME)

                                    'Add field type
                                    v_attrFLDTYPE = v_xmlTempDocument.CreateAttribute("fldtype")
                                    v_attrFLDTYPE.Value = CStr(CType(.Attributes.GetNamedItem("fldtype"), Xml.XmlAttribute).Value)
                                    v_nodeTemptxData.Attributes.Append(v_attrFLDTYPE)

                                    'Add current value
                                    v_attrOLDVAL = v_xmlTempDocument.CreateAttribute("oldval")
                                    v_attrOLDVAL.Value = CStr(CType(.Attributes.GetNamedItem("oldval"), Xml.XmlAttribute).Value)
                                    v_nodeTemptxData.Attributes.Append(v_attrOLDVAL)

                                    'Set value
                                    v_nodeTemptxData.InnerText = .InnerText
                                End With
                                v_dataTempElement.AppendChild(v_nodeTemptxData)
                            Next

                            v_dataTempElement.AppendChild(v_nodeTemptxData)
                            v_xmlTempDocument.DocumentElement.AppendChild(v_dataTempElement)
                            v_strXmlData = v_xmlTempDocument.InnerXml
                            Exit For
                        End If
                    Next
                Else
                    v_strTLTXCD = vbNullString
                End If
                frm.Dispose()
            Else
                Dim frm As New frmInquiry(Me.UserLanguage)
                frm.ObjectName = v_strTLTXCD
                frm.ModuleCode = Me.ModuleCode
                frm.LocalObject = gc_IsNotLocalMsg
                frm.BranchId = Me.BranchId
                ...
                frm.XmlObjData = v_strXmlData
                frm.ShowDialog()
                'Kiá»ƒm tra cÃ³ mÃ£ giao dá»‹ch tiáº¿p theo khÃ´ng
                v_strTLTXCD = frm.TLTXCD
                If Not Len(v_strTLTXCD) > 0 Then
                    v_strXmlData = vbNullString
                End If
                frm.Dispose()
            End If
    ...
    Public Overridable Sub ShowHistory(ByVal v_strTLTXCD As String, ByVal v_strXmlData As String)
        Try
            Dim frm As New frmHistory(Me.UserLanguage)
            frm.ObjectName = v_strTLTXCD
            frm.ModuleCode = Me.ModuleCode
            frm.LocalObject = gc_IsNotLocalMsg
            frm.BranchId = Me.BranchId
            ...
            frm.XmlObjData = v_strXmlData
            frm.HISTRORYCOMMAND = mv_strOrgRequestMessage
            frm.ShowDialog()
            frm.Dispose()
...
#Region " Form events "
                    ...
                    Else
                        If InStr(Me.ActiveControl.Name, PREFIXED_MSKDATA) > 0 Then
                            ...
                            If mv_arrObjFields(v_intIndex).LookupCheck = "Y" Then
                                SetLookUpDataForm()
                                Dim frmContractInfoScreen As New frmContractInfo(UserLanguage)
                                If Not TypeOf (ActiveControl) Is ComboBoxEx Then
                                    frmContractInfoScreen.AFACCTNO = ActiveControl.Text.Trim
                                Else
                                    frmContractInfoScreen.AFACCTNO = CType(ActiveControl, ComboBoxEx).SelectedValue
                                End If
                                frmContractInfoScreen.BranchId = Me.BranchId
                                frmContractInfoScreen.TellerId = Me.TellerId
                                frmContractInfoScreen.v_frmCFAUTH = Me.mv_frmCFAUTH
    ...
    Private Sub btnEntries_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEntries.Click
        Dim frm As New frmPostmap(UserLanguage)
        frm.mv_arrObjAccounts = mv_arrObjAccounts
        frm.mv_arrObjMISEntry = mv_arrObjIEFMIS
        frm.mv_arrObjVATVoucher = mv_arrObjVAT
        frm.BusDate = Me.BusDate
        frm.TellerId = Me.TellerId
        frm.BranchId = Me.BranchId
        If Me.btnOK.Visible = False Then
            frm.mv_blnGridReadOnly = True
        End If
        'Set transaction description for POSTMAP form from transaction form
        For i As Integer = 0 To mv_arrObjFields.Length - 1
            If (mv_arrObjFields(i).FieldName = "30") Then
                frm.TxDesc = CType(pnTransDetail.Controls(mv_arrObjFields(i).ControlIndex), TextBox).Text.Trim()
                Exit For
            End If
        Next i
        frm.ShowDialog()
        If Me.btnOK.Visible = True Then
            mv_arrObjAccounts = frm.mv_arrObjAccounts
            mv_arrObjVAT = frm.mv_arrObjVATVoucher
            mv_arrObjIEFMIS = frm.mv_arrObjMISEntry
        End If
        frm.Dispose()
        'Hiá»‡n bá»™ Ä‘á»‹nh khoáº£n
        RetrievePostmap()
    ...
    Private Sub btnVoucher_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnVoucher.Click
        ...
        If VoucherID.Trim.Length > 0 Then
            ...
            If v_arrVoucher.Length > 1 Then
                ...
                frmVouchers.TLTXCD = Me.mskTransCode.Text.Trim
                'frmVouchers.UserLanguage = Me.UserLanguage
                frmVouchers.ShowDialog()
                Dim v_returnvalue As String = Trim(frmVouchers.ReturnValue)
                PrintTransact(v_returnvalue)
            Else
                PrintTransact(VoucherID)
            End If
