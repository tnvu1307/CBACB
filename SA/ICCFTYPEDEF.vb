...
Imports System.Data
Public Class ICCFTYPEDEF
    Inherits CoreBusiness.Maintain

    Dim LogError As LogError = New LogError()

    Public Sub New()
        ATTR_TABLE = "ICCFTYPEDEF"
    End Sub
    ...
    Overrides Function Adhoc(ByRef v_strMessage As String) As Long
        ...
        Try
            ...
            End If
            v_strSQL = "UPDATE ICCFTYPEDEF SET DELTD='Y' WHERE TRIM(AUTOID)='" & v_strAUTOID & "'"
...
#Region " Overrides functions "
    ...
    Overrides Function SystemProcessBeforeDelete(ByRef v_strMessage As String) As Long
        ...
        Try
            ...
            Dim v_strLocal, v_strTLID As String
            ...
            v_strSQL = "insert into iccftier_hist " & ControlChars.CrLf _
                            ...
                            & " from iccftier WHERE MODCODE = '" & v_strModCode & "'" & ControlChars.CrLf _
            ...
            If v_strClause.Length > 0 Then
                v_strSQL = "insert into iccftypedef_hist " & ControlChars.CrLf _
                            & " (AUTOID,MODCODE,ACTYPE,GLACCTNO,EVENTCODE,RULETYPE,MONTHDAY,YEARDAY,PERIOD,PERIODDAY,ICTYPE,ICFLAT,ICRATECD,ICRATEID,ICRATE,MINVAL,MAXVAL,VARRATE,ICCFSTATUS,DELTD,OPERAND,LINE,FLRATE,CERATE,EFFECTIVEDT,BACKUPDT,MAKERID,ACTION) " & ControlChars.CrLf _
                            & " select AUTOID,MODCODE,ACTYPE,GLACCTNO,EVENTCODE,RULETYPE,MONTHDAY,YEARDAY,PERIOD,PERIODDAY,ICTYPE,ICFLAT,ICRATECD,ICRATEID,ICRATE,MINVAL,MAXVAL,VARRATE,ICCFSTATUS,DELTD,OPERAND,LINE,FLRATE,CERATE,EFFECTIVEDT,to_char(sysdate,'DD/MM/RRRR:HH24:MI:SS'), '" & v_strTLID & "', 'DELETE' ACTION " & ControlChars.CrLf _
                            & " from iccftypedef WHERE " & v_strClause
                v_obj.ExecuteNonQuery(CommandType.Text, v_strSQL)
            End If
    ...
    Overrides Function CheckBeforeAdd(ByVal v_strMessage As String) As Long
        ...
        Try
            ...
            'Kiểm tra Floor Rate, Ceiling rate không được vượt quá tầng System
            'If v_strRateCD = "F" Then
            '    v_strSQL = "SELECT IR.* FROM ICCFTYPEDEF IC, " & v_strModCode & "TYPE " & v_strModCode & ", IRRATE IR " & ControlChars.CrLf _
            '                & " WHERE IC.ICRATEID = IR.RATEID AND IC.ACTYPE = " & v_strModCode & ".ACTYPE AND IC.ACTYPE = '" & v_strACTYPE & "' AND IC.EVENTCODE = '" & v_strEVENTCODE & "'"

            '    v_ds = v_obj.ExecuteSQLReturnDataset(CommandType.Text, v_strSQL)
            '    If v_ds.Tables(0).Rows.Count > 0 Then
            '        Select Case v_strOperand
            '            Case "+"
            '                v_dblProductRate = v_ds.Tables(0).Rows(0)("RATE") + v_dblICRATE
            '            Case "-"
            '                v_dblProductRate = v_ds.Tables(0).Rows(0)("RATE") - v_dblICRATE
            '            Case "="
            '                v_dblProductRate = v_dblICRATE
            '        End Select

            '        If v_dblFLRate < v_ds.Tables(0).Rows(0)("FLRRATE") Or v_dblFLRate > v_ds.Tables(0).Rows(0)("CELRATE") Then
            '            Return ERR_SA_FLRATE_NOT_IN_SYSTEM_RATE_RANGE
            '        ElseIf v_dblCERate < v_ds.Tables(0).Rows(0)("FLRRATE") Or v_dblCERate > v_ds.Tables(0).Rows(0)("CELRATE") Then
            '            Return ERR_SA_CERATE_NOT_IN_SYSTEM_RATE_RANGE
            '        ElseIf v_dblProductRate < CDbl(v_ds.Tables(0).Rows(0)("FLRRATE")) Or CDbl(v_dblProductRate > v_ds.Tables(0).Rows(0)("CELRATE")) Then
            '            Return ERR_SA_ICRATE_NOT_IN_SYSTEM_RATE_RANGE
            '        End If
            '    End If
            'End If
            ...
            If v_strEVENTCODE.Length > 0 Then
                v_strSQL = "SELECT COUNT(ACTYPE) FROM ICCFTYPEDEF WHERE DELTD ='N' AND MODCODE = '" & v_strModCode & "'" & ControlChars.CrLf _
                 & " AND EVENTCODE = '" & v_strEVENTCODE & "' AND ACTYPE ='" & v_strACTYPE & "' AND DELTD ='N'"
                v_ds = v_obj.ExecuteSQLReturnDataset(CommandType.Text, v_strSQL)
    ...
    Overrides Function CheckBeforeEdit(ByVal v_strMessage As String) As Long
        ...
        Try
            Dim v_attrColl As Xml.XmlAttributeCollection = pv_xmlDocument.DocumentElement.Attributes
            Dim v_strLocal, v_strTLID, v_strClause, v_strACTYPE, v_strEVENTCODE, v_strAUTOID As String
            ...
            If v_strClause.Length > 0 Then
                v_strSQL = "insert into iccftypedef_hist " & ControlChars.CrLf _
                            & " (AUTOID,MODCODE,ACTYPE,GLACCTNO,EVENTCODE,RULETYPE,MONTHDAY,YEARDAY,PERIOD,PERIODDAY,ICTYPE,ICFLAT,ICRATECD,ICRATEID,ICRATE,MINVAL,MAXVAL,VARRATE,ICCFSTATUS,DELTD,OPERAND,LINE,FLRATE,CERATE,EFFECTIVEDT,BACKUPDT,MAKERID,ACTION) " & ControlChars.CrLf _
                            & " select AUTOID,MODCODE,ACTYPE,GLACCTNO,EVENTCODE,RULETYPE,MONTHDAY,YEARDAY,PERIOD,PERIODDAY,ICTYPE,ICFLAT,ICRATECD,ICRATEID,ICRATE,MINVAL,MAXVAL,VARRATE,ICCFSTATUS,DELTD,OPERAND,LINE,FLRATE,CERATE,EFFECTIVEDT,to_char(sysdate,'DD/MM/RRRR:HH24:MI:SS'), '" & v_strTLID & "', 'EDIT' ACTION " & ControlChars.CrLf _
                            & " from iccftypedef WHERE " & v_strClause
                v_obj.ExecuteNonQuery(CommandType.Text, v_strSQL)
            End If
            'ContextUtil.SetComplete()
