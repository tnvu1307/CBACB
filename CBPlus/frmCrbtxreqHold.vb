...
#Region " Khai báo hằng, biến "
    Const c_ResourceManager = "_DIRECT.frmCrbtxreqHold-"
                        ...
                        For v_intRow = 0 To gvSearchSelection.SelectedCount - 1
                            ...
                            If Not gvSearchSelection.GetSelectedRow(v_intRow) Is Nothing Then
                                ...
                                If v_strMODCODE <> "" And v_strTLTXCD <> "" Then
                                    ...
                                    mv_frmTransactScreen = New frmXtraTransact(mv_strLanguage)
...
#Region " Other methods "
    ...
    Protected Overridable Sub InitDialog()
        ...
        'loai 
        v_strCmdSQL = "SELECT CDVAL VALUECD,CDVAL VALUE,CDCONTENT DISPLAY , EN_CDCONTENT EN_DISPLAY,LSTODR FROM ALLCODE WHERE CDNAME = 'TYPECRBREQ' AND CDTYPE = 'OD'  ORDER BY LSTODR"
        ...
        'Me.grvsbfundetf.Columns.Clear()
        'Me.gridsbfundetf.DataSource = Nothing
        'gridsbfundetf.DataSource = InitGridSearchFields(cboType.EditValue, mv_arrSrFieldSrch, mv_arrSrFieldDisp, mv_arrSrFieldType)
        'InitGridFromSearchCode(cboType.EditValue, Me.gridsbfundetf, Me.grvsbfundetf, , "Y", "Y", UserLanguage)
        'bbiExecute.Visibility = BarItemVisibility.Always
        'Me.gridsbfundetf.RefreshDataSource()
        'Me.grvsbfundetf.RefreshData()
...
#Region " Các sự kiện của form "
    ...
    Private Sub gvResult_CustomDrawFooter(sender As Object, e As Views.Base.RowObjectCustomDrawEventArgs)

    End Sub

    Private Sub gvResult_CustomDrawRowIndicator(sender As Object, e As RowIndicatorCustomDrawEventArgs)

    End Sub

    Private Sub gvResult_RowCellStyle(sender As Object, e As RowCellStyleEventArgs)

        'If Me.ObjectName.ToUpper() = "CRBTXREQFULL" Then
        ...
        '        ElseIf gv.GetRowCellValue(e.RowHandle, "STATUS").ToString = "A" Then
