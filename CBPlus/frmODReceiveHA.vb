...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmODReceive))
        ...
        Me.lblDesc.Size = New System.Drawing.Size(54, 17)
        ...
        Me.btnViewTradingResult.UseVisualStyleBackColor = False
        '
        'txtEXECQTTY
        '
        ...
        Me.KeyPreview = True
        Me.Name = "frmODReceive"
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "frmODReceiveHA"
        Me.TabControl1.ResumeLayout(False)
        Me.tpgManual.ResumeLayout(False)
        Me.tpgManual.PerformLayout()
...
#Region " Declare constant and variables "
    Const c_ResourceManager = "_DIRECT.frmODReceiveHA-"
    Public ODReceiveGrid As GridEx
    Public ODAutoReceiveGrid As GridEx
    Dim v_objTrading_result As New TRADING_RESULTHA
    ...
    Dim mv_arrRusult() As TRADING_RESULTHA
    Dim mv_intResult As Integer
    Dim mv_arrOOD() As OODHA
    Dim mv_intOOD As Integer
    Dim mv_arrMatchingOrder() As MatchingOrderHA
                ...
                For v_intIndex = 0 To mv_arrObjFields.GetLength(0) - 1 Step 1
                    If Not mv_arrObjFields(v_intIndex) Is Nothing Then
                        ...
                        Select Case Trim(v_strFLDNAME)
                            ...
                            Case "10" 'PRICE                                       
                                If v_blAll Then
                                    v_strFLDVALUE = Convert.ToDouble(mv_strPRICE).ToString()
                                Else
...
#Region "Core function"
    ...
    '4: Khối lượng
    '------------------------------------------------------------------
    Private Function MatchingOrder(ByVal v_intOOD As Integer, ByVal v_intRESULT As Integer, ByRef v_intMatchingOrder As Integer, ByVal v_arrGo() As OODHA, ByVal v_arrBack() As TRADING_RESULTHA, ByRef v_arrMatching() As MatchingOrderHA) As Long
        Dim i, j As Integer
        ...
        Do While 1 = 1
            ...
            If v_arrBack(v_i).QUANTITY = 0 Then
                v_i = v_i + 1
            End If
            v_arrMatching(v_k) = New MatchingOrderHA
    ...
    Private Sub AutoMatching()
        ...
        Try
            ...
            v_strCMD = "SELECT ORGORDERID,CODEID,QTTY,CUSTODYCD,BORS,SYMBOL,NORP,AORN,ODPRICE,TXDATE, TXTIME,AFACCTNO, CIACCTNO, SEACCTNO, CLEARCD, CLEARDAY, TRADEUNIT, EXECQTTY, EXEC_PRICE,SECUREDRATIO,SORTPRICE FROM (SELECT OD.ORGORDERID, OD.CUSTODYCD, OD.BORS, OD.SYMBOL,OD.TXTIME, OD.TXDATE,OD.NORP,OD.CODEID,OD.AORN, " & ControlChars.CrLf _
                    ...
                    & "AND CD1.CDVAL=CD2.CDVAL AND CD1.CDVAL=SB.TRADEPLACE AND SB.CODEID=SECURITIES_INFO.CODEID AND SB.TRADEPLACE='002' " & ControlChars.CrLf _
            ...
            v_strCMD = "SELECT * FROM (SELECT   MATCH_DATE TXDATE, " & ControlChars.CrLf _
                        ...
                        & "  WHERE TRADING_RESULT.SEC_CODE = sbsecurities.symbol and sbsecurities.TRADEPLACE='002' and GRNAME = 'SYSTEM' AND VARNAME = 'COMPANYCD' AND QUANTITY-MATCHED_BQTTY>0 AND SUBSTR (B_ACCOUNT_NO, 1, 3) = VARVALUE  " & ControlChars.CrLf _
                        ...
                        & "  S_ORDER_NO, S_TRADING_ID, S_PC_FLAG, S_ACCOUNT_NO " & ControlChars.CrLf _
                        & "  FROM TRADING_RESULT, SYSVAR ,sbsecurities " & ControlChars.CrLf _
                        & "  WHERE TRADING_RESULT.SEC_CODE = sbsecurities.symbol and sbsecurities.TRADEPLACE='002' and GRNAME = 'SYSTEM' AND VARNAME = 'COMPANYCD' AND QUANTITY-MATCHED_SQTTY>0 AND SUBSTR (S_ACCOUNT_NO, 1, 3) = VARVALUE  " & ControlChars.CrLf _
            ...
            For i As Integer = 0 To v_nodeList.Count - 1
                ...
                mv_arrRusult(i) = New TRADING_RESULTHA
                For j As Integer = 0 To v_nodeList.Item(i).ChildNodes.Count - 1
                    With v_nodeList.Item(i).ChildNodes(j)
                        v_strValue = Trim(.InnerText.ToString)
                        v_strFLDNAME = CStr(CType(.Attributes.GetNamedItem("fldname"), Xml.XmlAttribute).Value)
...
#Region " Event "
    ...
    Private Sub frmODReceiveHA_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyUp
    ...
    Private Sub frmODReceiveHA_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#End Region
    ...
    Private Sub btnReceiveAll_ContextMenuChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReceiveAll.ContextMenuChanged
