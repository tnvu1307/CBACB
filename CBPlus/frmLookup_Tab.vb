...
#Region " Declare constant and variables "
    Const c_ResourceManager = "_DIRECT.frmLookup_Tab-"
...
#Region " Other methods "
        ...
        Try
            ...
            Else
                If Trim(SQLCMD) <> String.Empty Then
                    ...
                    Else
                        v_strObjMsg = BuildXMLObjMsg(, , , , gc_IsNotLocalMsg, gc_MsgTypeObj, OBJNAME_CF_CFMAST, gc_ActionInquiry, SQLCMD, "")
                        v_ws.Message(v_strObjMsg)
                    End If
                End If
            End If
    ...
    Private Sub OnAccept()
        ...
        If isReconcile Then
            ShowReconcileData()
        Else
            DisplayGeneralView() 'TanPN add 25/09/2020 mo form tra cuu
            'Me.Close()
        End If

    End Sub

    Private Sub ShowReconcileData()
        Dim frm As New AppCore.frmRECONCILE(Me.UserLanguage)
        Dim v_intPos As Integer, ctl As Control, v_strRETURNDATA, v_strObjName, v_strModCode As String

        Dim drv As DataRowView = grvLookup.GetFocusedRow()
        If drv IsNot Nothing Then
            frm.SQLCMD = drv.Row("SQLCHECK")
            If UserLanguage = "EN" Then
                frm.Text = drv.Row("EN_DISPLAY")
            Else
                frm.Text = drv.Row("DISPLAY")
            End If
            frm.AutoClosed = True
            frm.ShowDialog()
        End If
    ...
    Private Sub DisplayGeneralView()
        ...
        If strFrmRETURNDATA.Length <> 0 OrElse MenuTag.Length <> 0 Then
            If v_intPos > 0 OrElse MenuTag.Length <> 0 Then
                ...
                If v_strSTOREDNAME = "DFMRGRP" Then

                    'G?i màn hình tra cứu với SEARCHCODE=v_strRETURNDATA: Màn hình này chỉ cho phép tìm kiếm và thực hiện giao dịch kế tiếp
                    Dim frmSearch As New frmDFGroupView(UserLanguage)
                ...
                ElseIf v_strSTOREDNAME = "PO" Then
                    'G?i màn hình tra cứu với SEARCHCODE=v_strRETURNDATA: Màn hình này chỉ cho phép tìm kiếm và thực hiện giao dịch kế tiếp
                    Dim frmSearch As New frmPaymentOrder(UserLanguage)
                    frmSearch.BusDate = Me.BusDate
                    frmSearch.TableName = v_strObjName
                    frmSearch.CMDMenu = v_strObjName
                    frmSearch.ModuleCode = v_strModCode
                    frmSearch.AuthCode = "NNNNYYYYNN" 'Tren cac man hinh GView thi nut Xem ko co tac dung do do ko cho hien thi.
                    frmSearch.CMDTYPE = v_strCMDTYPE
                    frmSearch.IsLocalSearch = gc_IsNotLocalMsg
                    frmSearch.SearchOnInit = False
                    frmSearch.BranchId = Me.BranchId
                ...
                Else
                    ...
                    End If
                    If InStr("DF3001,DF3002,DF3003,DF3004,DF5009", v_strObjName) Then
                        frmSearch.AuthString = "YYYY"
                    End If
                    'FDS DieuNDA Begin
                    If InStr("DR00,DR01,DR02,DR03,DR04,DR06,DR07,DR12,DR40,DR60", v_strObjName) Then
                        frmSearch.AuthString = "YYYY"
                    End If
                    'FDS End 
                    frmSearch.CMDTYPE = v_strCMDTYPE
